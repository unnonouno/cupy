...
def make_rnn_workspace():
    cdef size_t handle = get_handle()
    cdef size_t work_size = libcudnn.getRNNWorkspaceSize(
        handle, rnn_desc.value, length, c_x_descs.data)
    cdef core.ndarray workspace = cuda.cupy.empty((work_size,), dtype='b')
    return workspace
    

def rnn_forward_inference_ex(rnn_desc, lengths, xs, hx, cx, w):
    c_x_descs = _make_tensor_descriptor_array(xs)
    cdef core.ndarray workspace = make_rnn_workspace()
